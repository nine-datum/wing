...
(fn [phys-world presets]
  (let [
      make-char (fn [preset pos dir color side] (dat/load-char phys-world preset pos dir color side :idle (core/get-time)))
      ...
      players (concat
        (map make-green (-> presets :ninja repeat) (range 1))
        (map make-red (-> presets :ninja repeat) (range 1))
      )
    ]
    players
  )
